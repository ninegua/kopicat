		...
		.mockImplementation(
			async (input: RequestInfo | URL, init?: RequestInit) => {
				...
				if (url.includes('/clip/') && init?.method === 'PUT') {
					try {
						const body: ClipInput = JSON.parse(
							init.body as string,
						);
						const id = body.id;
						...
						if (clipStore.has(id)) {
							return new Response(
								JSON.stringify('clip already exists'),
								{
									status: 403,
						...
						return new Response(
							JSON.stringify('Malformed JSON body'),
							{
								status: 400,
								headers: { 'Content-Type': 'application/json' },
							},
...
describe('Clip viewing flow', () => {
	...
	it('shows error when decryption fails with wrong password', async () => {
		...
		expect(get(clipState).error).toContain(
...
describe('UI component state flow', () => {
	...
	it('decrypt form: disables button when no password entered', () => {
		...
		expect(state.loading || !state.password).toBe(
			true,
